...
@property (nonatomic,strong)NSArray *images;            //保存多图情况下的图片名
...
/*
 *为了更好适配在多图的情况，我在源代码的基础上添加了一个传多图的方法
 */
+ (id)parallaxHeaderViewWithImages:(NSArray *)imageNames forSize:(CGSize)headerSize
{
    ...
    headerView.imageViews = [NSMutableArray arrayWithCapacity:imageNames.count];
//    headerView.headerImage = image;
    headerView.images = imageNames;
    [headerView initialSetupForImages];
    return headerView;
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    self.currentShowImageView = [self.imageViews objectAtIndex:index];
}

#pragma mark Private
- (void)initialSetupForImages
{
    UIScrollView *scrollView = [[UIScrollView alloc] initWithFrame:self.bounds];
    scrollView.contentSize = CGSizeMake(self.images.count*self.bounds.size.width, 0);
...
{
    UIScrollView *scrollView = [[UIScrollView alloc] initWithFrame:self.bounds];
    scrollView.contentSize = CGSizeMake(2*self.bounds.size.width, 0);
    scrollView.pagingEnabled = true;
    self.imageScrollView = scrollView;
